...
#確認檔案是否存在
yesterday_xlsx='NB_COVID_'+str(date.strftime(date.today()+timedelta(days=-1),'%m%d'))+'.xlsx'
today_xlsx='NB_COVID_'+str(date.strftime(date.today(),'%m%d'))+'.xlsx'

if '公費克流感使用清單.txt' in listdir(getcwd()) and yesterday_xlsx in listdir(getcwd()+'\COVID'):
    
    if path.isfile(getcwd()+'\\COVID\\'+today_xlsx)==False:
        #複製今日檔案
        copyfile(getcwd()+'\\COVID\\'+yesterday_xlsx,getcwd()+'\\COVID\\'+today_xlsx)
        try:
            #讀取系統匯出內容
            ...
            #開啟今日檔案寫入標籤
            filename=getcwd()+'\\COVID\\'+today_xlsx
            ...
            messagebox.showerror(title='發生錯誤',message='可能是空格滿了或者未把昨日檔案關閉無法讀取\n\n執行失敗')
    else:
        messagebox.showerror(title='已經執行過了',message='如果想要再執行一次\n\n就把今日檔案刪掉重來吧')
else:
    messagebox.showerror(title='發生錯誤',message='找不到所需的檔案\n\n\
公費克流感使用清單.txt\nCOVID資料夾內NB_COVID_XXXX.xlsx(昨日日期)\n兩者都必須存在')
root.destroy()
root.mainloop()